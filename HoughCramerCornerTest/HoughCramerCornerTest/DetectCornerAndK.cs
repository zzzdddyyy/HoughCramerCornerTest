...
namespace HoughCramerCornerTest
{
    ...
    public class DetectCornerAndK
    {
        ...
        public LineSegment2D[] GetLinesByHough(Bitmap img)
        {
            ...
            //二值化
            binaryImg = roiBinary.CopyBlank();//创建一张和灰度图一样大小的画布
            CvInvoke.Threshold(roiBinary, binaryImg, 250, 255, ThresholdType.Binary);
            //Closing
            ...
            for (int k = 0; k < contours.Size; k++)
            {
                ...
                if (area < 600000)//根据面积作筛选(指定最小面积,最大面积):
                {
        ...
        {
            //填充相机矩阵
            cameraMatrix[0, 0] = 5401.41696744257;
            cameraMatrix[0, 1] = 0;
            cameraMatrix[0, 2] = 272.52950509716516;
            cameraMatrix[1, 0] = 0;
            cameraMatrix[1, 1] = 5404.773735827147;
            cameraMatrix[1, 2] = 1049.1873155769958;
            cameraMatrix[2, 0] = 0;
            cameraMatrix[2, 1] = 0;
            cameraMatrix[2, 2] = 1;
            //填充畸变矩阵
            distCoeffs[0, 0] = -0.090094616667780444;
            distCoeffs[1, 0] = 0.15318326695809453;
            distCoeffs[2, 0] = -0.0033344017696337342;
            distCoeffs[3, 0] = 0.011883732684225462;
            distCoeffs[4, 0] = 0;
        }
    ...
    class SegmentsClass
    {
        ...
        public List<Segment> GroupLines(int Distance2origin, int AngleToleranceMin,int AngleToleranceMax)
        {
            ...
            if (Math.Abs(mySegment[1].az - mySegment[2].az) < 0.3)
            {
